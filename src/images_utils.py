...
def split_image(image: np.array, size_x: int = 128, size_y: int = 128) -> [np.array]:
...
def combine_images(images: [np.array], max_y: int, max_x: int) -> np.array:
...
def process_unet_img(img: np.array, model, batch_size: int = 20) -> np.array:
    ...
    parts = tmp
    img = combine_images(parts, img.shape[0], img.shape[1])
...
def binarize_img(img: np.array, model, batch_size: int = 20) -> np.array:
    """Binarize image, using U-net, Otsu, bottom-hat transform etc."""

    img = process_unet_img(img, model, batch_size)
